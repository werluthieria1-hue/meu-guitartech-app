...
def gerar_pdf_os(dados):
    ...
    pdf.ln(5)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 8, "SERVICOS CONTRATADOS", ln=True, fill=True)
    pdf.set_font("Arial", size=10)
    # Lista os serviços (separados por vírgula no banco)
    servicos_lista = dados['servico_nome'].split(", ")
    for s in servicos_lista:
        pdf.cell(0, 7, f"- {s.encode('latin-1', 'replace').decode('latin-1')}", ln=True)
    
    pdf.ln(3)
    pdf.set_font("Arial", 'I', 9)
    desc_limpa = dados['descricao_detalhada'].encode('latin-1', 'replace').decode('latin-1')
    pdf.multi_cell(0, 6, f"Obs: {desc_limpa}")
    
    pdf.ln(5)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 8, "RESUMO FINANCEIRO", ln=True, fill=True)
    pdf.set_font("Arial", size=11)
    pdf.cell(100, 8, f"Total dos Servicos:")
    pdf.cell(90, 8, f"R$ {dados['total']:.2f}", ln=True, align='R')
    pdf.cell(100, 8, f"Entrada Recebida:")
    pdf.cell(90, 8, f"R$ {dados['entrada']:.2f}", ln=True, align='R')
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(100, 10, "SALDO FINAL A PAGAR:")
    pdf.cell(90, 10, f"R$ {dados['total'] - dados['entrada']:.2f}", ln=True, align='R')
    
    return bytes(pdf.output(dest='S'))

# --- INTERFACE ---
st.title("🎸 GuitarTech ERP v5.0")

abas = st.tabs(["📄 Orçamentos", "⚙️ Faturar OS", "🛠️ Produção", "💰 Caixa", "⚙️ Catálogo"])

# 1. ABA DE ORÇAMENTOS (MÚLTIPLOS SERVIÇOS)
with abas[0]:
    ...
    with st.form("form_orc"):
        ...
        # Multiselect para escolher vários serviços do catálogo
        selecionados = st.multiselect(
            "Selecione os Serviços", 
            cat['nome'].tolist() if not cat.empty else ["Cadastre primeiro"]
        )
        
        # Cálculo automático baseado nos selecionados
        precos_sel = cat[cat['nome'].isin(selecionados)]['preco'].sum()
        desc_combinada = " / ".join(cat[cat['nome'].isin(selecionados)]['descricao_base'].tolist())
        
        detalhes = st.text_area("Observações Adicionais", value=desc_combinada)
        valor_final = st.number_input("Valor Total Sugerido (R$)", value=float(precos_sel))
        
        if st.form_submit_button("Salvar Orçamento"):
            servicos_texto = ", ".join(selecionados)
            conn = sqlite3.connect('oficina.db')
            conn.execute("INSERT INTO ordens (cliente, instrumento, servico_nome, descricao_detalhada, total, entrada, status, data_entrada) VALUES (?,?,?,?,?,?,?,?)",
                      (cliente, instrumento, servicos_texto, detalhes, valor_final, 0, 'Orçamento', datetime.now().date()))
            conn.commit()
            conn.close()
            st.success(f"Orçamento com {len(selecionados)} serviços salvo!")

# 2. FATURAMENTO (Mantido para transformar em OS)
with abas[1]:
    st.header("Faturar Orçamento")
    conn = sqlite3.connect('oficina.db')
    pendentes = pd.read_sql_query("SELECT id, cliente, instrumento, servico_nome FROM ordens WHERE status='Orçamento'", conn)
    conn.close()

    if not pendentes.empty:
        escolha_id = st.selectbox("Selecione o Orçamento", pendentes['id'].tolist(), 
                                  format_func=lambda x: f"ID {x} - {pendentes[pendentes['id']==x]['cliente'].values[0]} ({pendentes[pendentes['id']==x]['servico_nome'].values[0]})")
        
        with st.container(border=True):
            data_ent = st.date_input("Data de Entrega")
            tecnico = st.text_input("Técnico Responsável", value="Regis")
...
# 3. PRODUÇÃO, 4. CAIXA, 5. CATÁLOGO (Mantidos)
with abas[2]:
    st.header("🛠️ Produção")
    conn = sqlite3.connect('oficina.db')
    df_prod = pd.read_sql_query("SELECT id, cliente, instrumento, servico_nome as Servicos, data_entrega FROM ordens WHERE status='Em Produção'", conn)
    conn.close()
    if not df_prod.empty:
        st.dataframe(df_prod, use_container_width=True)
        id_pdf = st.number_input("ID para PDF", min_value=1, step=1)
        if st.button("📄 Gerar PDF"):
            conn = sqlite3.connect('oficina.db')
            dados_os = pd.read_sql_query(f"SELECT * FROM ordens WHERE id={id_pdf}", conn).iloc[0]
            conn.close()
            st.download_button("Baixar PDF", gerar_pdf_os(dados_os), file_name=f"OS_{id_pdf}.pdf")
...
with abas[3]:
    ...
    conn.close()
    st.metric("Total Entradas", f"R$ {res['e'][0] or 0:.2f}")
